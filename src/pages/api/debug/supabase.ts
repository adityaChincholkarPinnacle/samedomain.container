...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    try {
      ...
      summary['base_error'] = e?.message || String(e);
    }
    ...
    try {
      ...
      summary['rest_error'] = e?.message || String(e);
    }
    ...
    try {
      ...
      sbOk = `exception: ${e?.message || String(e)}`;
    }

    return res.status(200).json({ summary, baseStatus, restStatus, sbOk });
  } catch (err: any) {
    return res.status(500).json({ message: 'debug route error', error: err?.message || String(err) });
  }
}
